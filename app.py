...
def init_firebase_admin():
    ...
    try:
        # Carga el string JSON completo desde los secretos
        creds_json_str = st.secrets["firebase_secrets"]["credentials_json"]
        
        # Convierte el string a un diccionario de Python
        creds_dict = json.loads(creds_json_str)
        
        # Corrige el formato de la clave privada reemplazando '\\n' por '\n'
        creds_dict['private_key'] = creds_dict['private_key'].replace('\\n', '\n')

        # Inicializa la aplicación con el diccionario
...
# Solo inicializa las apps si no se han inicializado antes
...
# --- FUNCIONES DE MANEJO DE DATOS CON FIRESTORE ---
def df_to_json(df):
...
def save_data_to_firestore():
    """Guarda todo el estado de la sesión del usuario en Firestore."""
    ...
    try:
        doc_ref = db.collection('users').document(user_id).collection('app_data').document('main')
        doc_ref.set(data_to_save)
    except Exception as e:
        st.error(f"Error al guardar los datos en la nube: {e}")

def load_data_from_firestore():
    """Carga los datos del usuario desde Firestore y los popula en el estado de la sesión."""
    if 'user_id' not in st.session_state or not st.session_state.user_id:
        return
    user_id = st.session_state.user_id
    doc_ref = db.collection('users').document(user_id).collection('app_data').document('main')
...
# --- LÓGICA DE AUTENTICACIÓN Y PANTALLA DE LOGIN ---
def show_login_page():
    st.title("🚀 Centro de Mando de Ofertas")
    st.subheader("Bienvenido a la Versión Academia")
...
# --- Se omite por brevedad, ya que es idéntico a la versión anterior ---
# --- ... (pegar el resto de las funciones desde la versión anterior) ... ---
...
def main_app():
    with st.sidebar:
        ...
        if st.button("Cerrar Sesión", use_container_width=True, type="secondary"):
            ...
            # Limpia todos los datos de la sesión para el siguiente usuario
        ...
        st.divider()
        st.info("Tus datos se guardan automáticamente en la nube.")
        ...
        if st.session_state.vista_actual == 'dashboard':
            ...
            if not st.session_state.get('ofertas'): st.info("Aún no has creado ninguna oferta.")
            else:
