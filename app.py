...
if 'oferta_seleccionada' not in st.session_state:
    st.session_state['oferta_seleccionada'] = None
if 'anuncio_para_escalar' not in st.session_state:
    st.session_state['anuncio_para_escalar'] = None
if 'accion_de_escala' not in st.session_state:
    st.session_state['accion_de_escala'] = None
if 'editing_record' not in st.session_state:
    st.session_state['editing_record'] = None
if 'offer_to_delete' not in st.session_state:
    st.session_state['offer_to_delete'] = None
if 'editing_boveda_id' not in st.session_state:
    st.session_state['editing_boveda_id'] = None
if 'boveda_view_mode' not in st.session_state:
    st.session_state['boveda_view_mode'] = '🖼️ Tarjetas'
if 'plantillas' not in st.session_state:
    st.session_state['plantillas'] = {}
if 'editing_plantilla_id' not in st.session_state:
    st.session_state['editing_plantilla_id'] = None
if 'editing_checklist_oferta_id' not in st.session_state:
    st.session_state['editing_checklist_oferta_id'] = None # Para editar checklists de ofertas
...
# --- NUEVA FUNCIÓN: Callback para actualizar el estatus desde la tarjeta ---
def update_boveda_status(entrada_id, key):
    """
    Esta función se activa cuando se cambia el estatus en un selectbox de una tarjeta.
    Actualiza el estatus de la oferta correspondiente en st.session_state.
    """
...
def procesar_archivo_cargado(archivo):
    try:
        datos_cargados = pickle.load(archivo)
        
        if isinstance(datos_cargados, dict):
            if 'ofertas' in datos_cargados and 'boveda' in datos_cargados:
                st.session_state.ofertas = datos_cargados['ofertas']
                st.session_state.boveda = datos_cargados['boveda']
                st.session_state.plantillas = datos_cargados.get('plantillas', {})
            else:
                st.session_state.ofertas = datos_cargados
                st.session_state.boveda = []
                st.session_state.plantillas = {}
            
            st.success("¡Datos cargados con éxito! Bienvenido de vuelta.")
            seleccionar_oferta(None)
        else:
            st.error("Error: El archivo no contiene datos válidos para esta aplicación.")
...
with st.sidebar:
    ...
    if st.button("📈 Mis Ofertas y Dashboard", use_container_width=True):
        st.session_state.vista_actual = 'dashboard'
        st.session_state.oferta_seleccionada = None
        st.rerun()
    ...
    try:
        datos_para_guardar = pickle.dumps({
            'ofertas': st.session_state.ofertas,
            'boveda': st.session_state.boveda,
            'plantillas': st.session_state.plantillas
        })
        st.download_button(
            label="💾 Guardar Progreso",
            data=datos_para_guardar,
            file_name=f"centro_de_mando_datos_{datetime.date.today()}.pkl",
            mime="application/octet-stream",
            use_container_width=True,
            help="Descarga un archivo con todos los datos de tus ofertas, la bóveda y las plantillas."
        )
    except Exception as e:
        st.error(f"Error al preparar los datos para guardar: {e}")

    archivo_cargado = st.file_uploader(
        "📂 Cargar Progreso",
        type="pkl",
        help="Sube un archivo .pkl para restaurar tu sesión.",
        key="cargador_archivos"
    )
    if archivo_cargado:
        if 'ultimo_archivo_cargado' not in st.session_state or st.session_state.ultimo_archivo_cargado != archivo_cargado.name:
            procesar_archivo_cargado(archivo_cargado)
            st.session_state.ultimo_archivo_cargado = archivo_cargado.name
            st.rerun()
    ...
    if st.session_state.vista_actual == 'dashboard':
        ...
        st.divider()
        st.header("🗂️ Mis Ofertas")
        if not st.session_state.ofertas: st.info("Aún no has creado ninguna oferta.")
        else:
...
if st.session_state.vista_actual == 'plantillas':
    ...
    plantilla_to_edit = st.session_state.plantillas.get(editing_id_plantilla)
                ...
                if st.form_submit_button("💾 Guardar Plantilla" if not editing_id_plantilla else "💾 Actualizar Plantilla", use_container_width=True):
                    if nombre_plantilla and checklist_texto:
                        if editing_id_plantilla:
                            ...
                            if id_plantilla in st.session_state.plantillas:
    ...
    st.divider()
    st.header("🗂️ Mis Plantillas Guardadas")

    if not st.session_state.plantillas:
    ...
    # --- MODIFICACIÓN: Patch para añadir 'estatus' a entradas antiguas ---
    ...
    # --- CAMBIO: Aumento de la intensidad del 'glow' ---
    st.markdown("""
    <style>
        .card-boveda { 
            border-radius: 5px; 
        ...
        .glow-modelando {
            /* CAMBIO: Aumentado el blur a 25px y el spread a 8px */
            box-shadow: 0 0 25px 8px #ffc107aa;
        }
        .glow-pruebas {
            /* CAMBIO: Aumentado el blur a 25px y el spread a 8px */
            box-shadow: 0 0 25px 8px #28a745aa;
        }
        .fade-archivada {
            opacity: 0.6;
        }
    ...
    st.title("📈 Dashboard Principal del Negocio")
    st.markdown("Visión general del rendimiento de todas tus ofertas activas.")

    if not st.session_state.ofertas:
        st.info("Crea tu primera oferta en la barra lateral para empezar a ver datos aquí.")
    else:
        all_dfs = []
        ...
        if not all_dfs:
            st.warning("No hay datos registrados en ninguna de tus ofertas activas.")
        else:
            ...
            df_filtrado = df_global[
                (df_global['Fecha'].dt.date >= start_date_global) &
                (df_global['Fecha'].dt.date <= end_date_global)
            ]
            ...
            if df_filtrado.empty:
                ...
                df_filtrado['Comisiones'] = df_filtrado.apply(
                    lambda row: row.get(ventas_pp_col, 0) * row.get('Comision PP', 0),
                    axis=1
                )
                ...
                df_por_oferta = df_filtrado.groupby('Oferta').agg({
                    'Inversión': 'sum',
                    'Facturación Total': 'sum',
                    'Ganancia Neta': 'sum'
                }).reset_index()
                
                st.dataframe(df_por_oferta.style.format({
                    'Inversión': "${:,.2f}",
                ...
                with st.expander("📅 Análisis de Rendimiento por Día de la Semana"):
                    ...
                    df_por_dia = df_analisis_dia.groupby('Día de la Semana', observed=False).agg({
                        'Inversión': 'sum',
                        'Ganancia Neta': 'sum'
                    }).reset_index()
                    ...
                    st.dataframe(
                        df_por_dia.style.format({
                            'Inversión': "${:,.2f}",
                            'Ganancia Neta': "${:,.2f}"
                        }).apply(lambda row: ['background-color: #e6ffed; color: black;' if row['Ganancia Neta'] > 0 else 'background-color: #ffe6e6; color: black;' for i in row], axis=1),
                        use_container_width=True
                    )
# --- CÓDIGO RESTAURADO ---
    ...
    try:
        ...
        with st.form("form_edit_record", clear_on_submit=False):
            ...
            for item_details in funnel_activo.values():
                col_name = get_safe_column_name(item_details['alias'])
                nuevas_ventas[col_name] = form_cols[i].number_input(
                    item_details['alias'],
                    value=int(registro_a_editar.get(col_name, 0)),
                    min_value=0, step=1, key=f"edit_venta_{item_details['alias']}"
                )
                i += 1
            ...
            if col_submit.form_submit_button("💾 Guardar Cambios", use_container_width=True):
                registro_actualizado = {
                    "Fecha": registro_a_editar['Fecha'],
                    "Inversión": nuevo_inversion,
                    **nuevas_ventas
                }
                if is_test_record:
    ...
    with tab_campanas:
        ...
        with sub_tab_test:
            ...
            if df_testeos_global.empty: st.info("Aún no hay datos para analizar.")
                ...
                if not df_filtrado_diario.empty:
                    df_agrupado = df_filtrado_diario.groupby("Anuncio").agg({
                        'Inversión': 'sum', 'Pagos Iniciados': 'sum', 'Facturación Total': 'sum', 'Ganancia Neta': 'sum',
                        **{get_safe_column_name(v['alias']): 'sum' for v in oferta_actual['funnel'].values() if get_safe_column_name(v['alias']) in df_filtrado_diario}
                    }).reset_index()
                    precio_pp = oferta_actual['funnel']['principal']['precio']
                    ventas_pp_col = get_safe_column_name("PP")
                    ...
                    with st.container(border=True):
                        ...
                        if not anuncios_activos_para_escala:
                            ...
                            anuncio_manual_a_escalar = st.selectbox(
                                "Selecciona cualquier anuncio ACTIVO para escalar",
                                options=anuncios_activos_para_escala,
                                index=None,
                                placeholder="Elige un anuncio para el lanzamiento directo...",
                                key="lanzamiento_directo_sb"
                            )
                    ...
                    with st.expander(expander_title, expanded=True):
                        ...
                        with col_gest_1:
                            ...
                            if not componentes_totales:
                                ...
                                componente_a_gestionar = st.selectbox(
                                    "Seleccionar componente para cambiar estado",
                                    options=componentes_totales,
                                    key=f"sb_gest_comp_{cid}"
                                )
                        with col_gest_2:
                        ...
                        if not df_filtrado_escala.empty:
                            df_agrupado_escala = df_filtrado_escala.groupby("Componente").agg({
                                'Inversión': 'sum', 'Pagos Iniciados': 'sum', 'Facturación Total': 'sum', 'Ganancia Neta': 'sum',
                                 **{get_safe_column_name(v['alias']): 'sum' for v in oferta_actual['funnel'].values() if get_safe_column_name(v['alias']) in df_filtrado_escala}
                            }).reset_index()
                            precio_pp = oferta_actual['funnel']['principal']['precio']
                            ventas_pp_col = get_safe_column_name("PP")
                            ...
                            if df_para_mostrar_escala.empty:
                                ...
                                st.dataframe(df_display_escala.style
                                    .apply(lambda x: x.map(color_roas), subset=['ROAS FE', 'ROAS Total (Neto)'])
                                    .apply(lambda x: x.map(color_ganancia), subset=['Ganancia Neta'])
                                    .format({'Inversión': "${:,.2f}", 'Ganancia Neta': "${:,.2f}", 'CPA': "${:,.2f}", 'ROAS FE': "{:.2f}", 'ROAS Total (Neto)': "{:.2f}"}),
                                    use_container_width=True)
                            ...
                            if len(componentes_con_datos) == 0:
                                ...
                                componente_a_desglosar = st.selectbox(
                                    "Selecciona un Componente para ver su desglose diario",
                                    options=componentes_con_datos,
                                    key=f"sb_desglose_escala_{cid}"
                                )
        ...
        with sub_tab_analisis:
            ...
            if df_consolidado_funnel.empty:
                ...
                df_filtrado_funnel = df_consolidado_funnel[
                    (df_consolidado_funnel['Fecha'].dt.date >= start_date_f) & 
                    (df_consolidado_funnel['Fecha'].dt.date <= end_date_f)
                ]
                ...
                if not df_filtrado_funnel.empty:
                    ...
                    agg_dict = {
                        ...
                        'Facturación Total': ('Facturación Total', 'sum'),
                        ventas_pp_col: (ventas_pp_col, 'sum')
                    }
                    ...
                    if agrupacion == "Día":
                        ...
                        st.dataframe(df_agrupado_temp[display_cols].style.format({
                            'Inversión': "${:,.2f}", 'Facturación Total': "${:,.2f}", 'Facturación FE': "${:,.2f}",
                            'Ganancia Neta': "${:,.2f}", 'Ganancia Neta FE': "${:,.2f}", 'ROAS Neto': "{:.2f}", 'ROAS FE': "{:.2f}"
                        }), use_container_width=True)
                        ...
                        for index, row in df_agrupado_periodo.iterrows():
                            ...
                            with st.expander(expander_title):
                                ...
                                st.dataframe(df_desglose_diario[display_cols_desglose].sort_values(by="Fecha").style.format({
                                    'Inversión': "${:,.2f}", 'Facturación Total': "${:,.2f}", 'Facturación FE': "${:,.2f}",
                                    'Ganancia Neta': "${:,.2f}", 'Ganancia Neta FE': "${:,.2f}", 'ROAS Neto': "{:.2f}", 'ROAS FE': "{:.2f}"
                                }), use_container_width=True)
